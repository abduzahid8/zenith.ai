...
export const chessLessonPuzzles: Record<string, ChessPuzzle[]> = {
  ...
  day4_castling: [
    ...
    // ── Задача 3 ── Одноходовая | best_move
    {
      ...
      goalType: "best_move",
      fen: "r1bqk2r/ppp2ppp/2n2n2/3pp3/1bBPP3/2N2N2/PPP2PPP/R1BQK2R w KQkq - 0 7",
      sideToMove: "w",
      solution: [
        {
          userMove: "e1g1",
          opponentMove: null,
          explanation: "Рокировка важнее, чем взятие слона на b4 — король уходит в безопасное место."
        }
      ],
      prompt: "Твою фигуру атакуют, но король в центре. Что важнее: взять фигуру или защититься?",
      hints: {
        soft: "Оцени, что будет, если взять атакуемую фигуру прямо сейчас.",
        medium: "После взятия центр вскроется. Где окажется твой король?",
        strong: "Есть ход, который решает проблему короля и не оставляет его под ударом."
      },
      successExplanation: "Рокировка важнее взятия. После 0-0 король в безопасности, а материальные потери можно компенсировать позже.",
      failureExplanation: "Взяв на b4, ты выиграл слона, но после вскрытия центра король остаётся под ударами. Жадность наказывается.",
      learningPoint: "Не бери материал, если это оставляет короля под атакой. Безопасность короля — приоритет.",
  ...
  day5_check_mate_stalemate: [
    {
      ...
      moves: ["g1e3"],
      prompt: "Твой король оказался под шахом от ладьи. Найди способ перекрыть эту линию атаки.",
      hints: [
    ...
    },
    {
      ...
      moves: ["e1f1"],
      prompt: "На твоего короля совершено нападение ладьёй. Найди безопасную клетку для отхода.",
      hints: [
    ...
    },
    {
      ...
      moves: ["f7f8"],
      prompt: "Король противника зажат в углу доски. Найди способ объявить ему неизбежный мат.",
      hints: [
    ...
    },
    {
      ...
      moves: ["f1f8"],
      prompt: "Король черных отрезан на последней линии доски. Нанеси решающий удар своей ладьёй.",
      hints: [
    ...
    },
    {
      ...
      moves: ["g1g7"],
      prompt: "Используй совместные силы своего короля и ферзя, чтобы поставить мат в один ход.",
      hints: [
    ...
    },
    {
      fen: "7k/8/5K2/8/8/8/8/6Q1 w - - 0 1",
      moves: ["g1g6"],
      prompt: "Будь предельно внимателен! Сделай ход, который лишает противника ходов без шаха.",
      hints: [
    ...
    },
    {
      ...
      moves: ["f1f8"],
      prompt: "Черный король заперт в углу. Поставь мат в один ход, не давая сопернику спастись.",
      hints: [
    ...
    },
    {
      fen: "7k/8/6K1/8/8/8/8/5Q2 w - - 0 1",
      moves: ["f1f7"],
      prompt: "Покажи пример ошибочного хода, который оставляет черных без ходов в ничейном пате.",
      hints: [
